...
import MigrosYemekCancelOrderPopup from "./migrosYemekCancelOrderPopup";

const MigrosYemekStatusButtons = ({ order, setOrdersData, setSideOrder }) => {
  ...
  const [prepareDisabled, setPrepareDisabled] = useState(
    disabled || order.status != 2 || xMinWait(order.approvalDate)
  );
  const [deliverDisabled, setDeliverDisabled] = useState(
    disabled || order.status != 6 || xMinWait(order.preparationDate, 10)
  );
  const [cancelDisabled, setCancelDisabled] = useState(
    disabled || order.status == 8 || order.status == 9 || order.cancelDate
  );
...
export default MigrosYemekStatusButtons;
